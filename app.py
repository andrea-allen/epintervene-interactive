...
st.text("Epidemic SIR Simulations on a 100-node network")
...
num_sims = st.radio("Number of simulations", ("1", "10", "100", "1000"))
...
line, = ax.plot(timeseries_results_cum, nanfilled, color=hex_list[3])
line_rec, = ax.plot(timeseries_results_cum, nanfilled_rec, color=hex_list[2])
ax.set_ylim(0, max(infected_results)+5)
...
def animate(i):  # update the y values (every 1000ms)
    ...
    # line.set_ydata(infected_results[i])
    the_plot.pyplot(fig)
